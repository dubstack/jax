...
```{note}
XProf is the underlying tool that powers the "Profile" tab in TensorBoard.
As long as `xprof` is installed, a "Profile" tab will be present within
Tensorboard. Using this is identical to launching XProf independently.
```
